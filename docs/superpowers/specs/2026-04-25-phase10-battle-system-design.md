...
> **Phase 10.2.E 補記**: 10.2.E で `OwnedRelicIds: ImmutableArray<string>` /
> `Potions: ImmutableArray<string>` を追加。配置は `EncounterId` の直前。
> `Start` 時に `RunState.Relics`（DefinitionId 配列）と `RunState.Potions`（DefinitionId 配列、空スロット `""` 込み）から
> snapshot を取得。`OwnedRelicIds` は戦闘中不変（レリック取得は報酬フェーズのみ）、
> `Potions` は `UsePotion` で消費スロットを `""` に置換。
> `Finalize` 時に `state.Potions` を `RunState.Potions` に丸ごとコピーして反映。
...
> **Phase 10.2.E 補記**: 10.2.E で `BattleEngine.Start` 末尾に `OnBattleStart` レリック発動を追加。
> 実行順序: ターン 1 開始処理 (`TurnStartProcessor.Process`) → `OnBattleStart` レリック発動
> → 対象初期化 → return。
> OnBattleStart レリック発動で生成された `BattleEvent` 列は `Start` の戻り値タプルに含まれる
>（`Start` シグネチャ変更: §4-3 補記参照）。
...
> **Phase 10.2.E 補記**: 10.2.E で `OnTurnStart` レリック発動（step 7）を実装。
> ドロー（step 6）の直後 / `TurnStart` event 発火の前に挿入（step 8 とも呼ぶ）。
> 親 spec の "step 7: OnTurnStart レリック発動" は「Draw 後」の位置として明確化。
> 実装上は `TurnStartProcessor.Process` 末尾で `RelicTriggerProcessor.FireOnTurnStart` を呼び出す。
> OnTurnStart レリック発動で生じた `BattleEvent` は同一 events リストに追記される。
...
> **Phase 10.2.E 補記**: 10.2.E で `UsePotion` を**第 6 公開 API** として追加。
> `BattleEngine` の公開 API は計 6 本: `Start` / `PlayCard` / `EndTurn` / `Finalize` / `SetTarget` / `UsePotion`。
> 合わせて `Start` のシグネチャを `(BattleState state, IReadOnlyList<BattleEvent> events)` の
> タプル返却に変更（破壊的変更）。変更前は `BattleState` 単体返却だった。
> 戦闘開幕に発火された `BattleEvent`（OnBattleStart レリック等）を呼出側に渡すために必要。
...
> **Phase 10.2.E 補記**: 10.2.E で `OnTurnEnd` レリック発動（step 3）を実装。
> AttackPool リセット（step 2）の直後 / コンボリセット（step 4）の前に挿入。
> 合わせて `TurnEndProcessor.Process` のシグネチャに `IRng rng` 引数を追加
>（`RelicTriggerProcessor.FireOnTurnEnd` の呼び出しに乱数が必要なため。既存 caller は
> `BattleEngine.EndTurn` が保持する `IRng` を渡す）。
...
> **Phase 10.2.E 補記**: 10.2.E で `EffectApplier.Apply` のシグネチャは変更なし。
> `BattleOnly` フラグは戦闘内（`BattleEngine` 経由）では常に無視して effect を適用する。
> 戦闘外でのポーション使用は Phase 10.5 で `OutOfBattleEffectApplier` を新設して対応予定
>（`BattleOnly: true` の effect をスキップするラッパー）。
> `EffectApplier.Apply` 自体は戦闘外/戦闘内を意識しない pure function を維持する。
...
> **Phase 10.2.E 補記**: 戦闘外ポーション使用の実装は **Phase 10.5 のスコープ**。
> 10.2.E では戦闘内 `UsePotion` のみ実装し、戦闘外 UI・ロジックは触らない。
> 10.5 実装時は `OutOfBattleEffectApplier` を新設して `BattleOnly: true` effect をスキップする
> ラッパーとして実装する予定（§5-1 補記参照）。
...
> **Phase 10.2.E 補記**: 10.2.E で `UsePotion` を**第 6 公開 API** として実装。
> Phase=PlayerInput 限定（他 Phase で `InvalidOperationException`）、コスト消費なし、コンボ更新なし、捨札移動なし。
> 指定 slot の `Potions[index]` が `""` の場合は `InvalidOperationException`。
> 消費後は `state.Potions[index]` を `""` に置換。
> 対象切替は `PlayCard` と同じ `target ?? state.Target` パターンを踏襲:
> `targetEnemyIndex ?? state.TargetEnemyIndex` / `targetAllyIndex ?? state.TargetAllyIndex` を
> `EffectApplier.Apply` に渡す（一時的に state を更新して effect 後に元の Target に戻す方式ではない）。
...
> **Phase 10.2.E 補記**: 10.2.E で戦闘内 `UsePotion` を実装（第 6 公開 API として BattleEngine に追加）。
> `PotionDefinition.Effects` を `CardEffect` 配列に統一し、`EffectApplier.Apply` を共通利用。
> caster は主人公（`Allies.FirstOrDefault(a => a.DefinitionId == "hero")`）。
> 戦闘外ポーション UI・ロジックは Phase 10.5 のスコープ（§5-6 補記参照）。
...
> **Phase 10.2.E 補記**: 10.2.E で 4 Trigger の全 6 発火サイトを実装。
> 具体的な発火位置:
> - `OnBattleStart`: `BattleEngine.Start` 末尾（ターン 1 開始処理後、対象初期化前）
> - `OnTurnStart`: `TurnStartProcessor.Process` 末尾（Draw 後 = step 8）
> - `OnTurnEnd`: `TurnEndProcessor.Process` 内（AttackPool reset 後 / コンボリセット前 = step 3）
> - `OnCardPlay`: `BattleEngine.PlayCard` の effect 適用後・カード移動ロジック前
> - `OnEnemyDeath` (攻撃発射ごと): `PlayerAttackingResolver` 内で各攻撃発射後に新たな死亡 actor を検出
> - `OnEnemyDeath` (毒死): `TurnStartProcessor.ApplyPoisonTick` 内で毒死判定後に発動
...
> **Phase 10.2.E 補記**: 10.2.E で caster 索引を `Allies[0]` 直接参照から
> `Allies.FirstOrDefault(a => a.DefinitionId == "hero")` による defensive 検索に変更。
> `Allies[0]` 直参照は将来的な Allies 順序変更に対して脆弱であるため。
> hero 不在（`== null`）の場合はレリック発動を silent skip（戦闘終了状態と判断）。
> この索引方式は `BattleEngine.UsePotion` / `BattleEngine.Finalize` でも同様に採用
>（memory feedback ルール「InstanceId で検索」の精神に準拠）。
...
> **Phase 10.2.E 補記**: 10.2.E で `UsePotion = 19` を追加（計 20 値）。
> ペイロード慣例:
> - `UsePotion`: Caster=hero、Amount=null、Note=potionDefinitionId
>
> レリック発動の events は専用 `BattleEventKind` を持たず、既存 Kind のイベントの `Note` 末尾に
...
> **Phase 10.2.E 補記**: 10.2.E で `BattleSummary` に `ConsumedPotionIds: ImmutableArray<string>` を追加。
> `Finalize` 時に `before.Potions`（戦闘開始前の RunState.Potions）と `state.Potions`（戦闘終了時）の
> diff（`before` にあって `state` では `""` になった DefinitionId）として算出。
> `RunState.Potions` への書き戻しは `state.Potions` を丸ごとコピーする方式（差分更新ではなく全置換）。
> hero 不在（`Allies` に DefinitionId=="hero" の actor が存在しない）の場合は例外を投げずに
> `ConsumedPotionIds = ImmutableArray<string>.Empty` として継続。
